...
if __name__ == "__main__":
	...
	for k in xrange(n_skies):
		...
		position_halo[k,0,0] = np.where(mass == \
						np.max(mass))[0][0]*binwidth
						#I find the maximum likelihood point
						#and its associated index
		position_halo[k,1,0] = np.where(mass == \
						np.max(mass))[1][0]*binwidth
						#and then find which x and y position
						#this is in the sky.
		
	
	c = c.writer(open("../Data/Matched_Filter_Benchmark_Test.csv", "wb")) #Now write the array to a csv file
